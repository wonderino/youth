...
var update;
var colorList = ['#f2cac7','#e5918a','#d13b2f'] //['#e8a39e','#d13b2f','#991b1e']//['#96abc6', '#f68c54', '#d13c2d']
...
function drawLinks(keyword) {
  ...
  .style('stroke', function(d) {
    ...
    return colorList[Number(d.level)-1]
  })

  link.style('opacity', 0).transition().delay(100).duration(800).style('opacity', 1).attr('d', d3.svg.diagonal()
  .source(function(d) { return {"x":d.source.y+rowHeight+ 16, "y":d.source.x + d.source.keyword.length*8}; })
  .target(function(d) { return {"x":d.target.y+rowHeight+ 16, "y":d.target.x-d.target.keyword.length*8}; })
...
function drawKeywordList(className, threeData, isFromRight, isNew) {
  ...
  .attr("transform", function(d,i) {
    ...
    return "translate(" + x + "," + (margin.top+32) + ")";
  })
  canvas.transition().duration(400)
  .attr("transform", function(d,i) {
    return "translate(" + (i*innerWidth) + "," + (margin.top+32) + ")";
  }).each(function(d,i) {
  ...
  .attr('text-anchor', 'middle')
  .style('fill', function(d) {return colorList[Number(d.level)-1]})
  .text(function(d) {return d.keyword})
  .on('click', function(d) {
    var thisKeyword = d.keyword;
    var isSelected = d3.select(this).classed('selected');
    d3.selectAll('.list').selectAll('text').classed({selected:false})
    d3.selectAll('.link').remove();
    canvas.selectAll('.bbox.keyword').remove();
    if (!isSelected) {
  ...
  if (isNew || d3.selectAll('.list').selectAll('text')) {
    canvas.selectAll('text')
  }
...
function drawHalfYearText(className, threeData, ratios, isFromRight) {
  ...
  .attr('y', function() {return margin.top+rowHeight})
  .attr('dy', '.35em')
  .attr('text-anchor', 'middle')
  .text(function(d, i) {
...
.attr('text-anchor', function(d,i) {
  ...
  function drawLegend() {
    var legendW = Math.floor(width * 0.075);
    var xFrom = (width - colorList.length*legendW )/2

    svg.selectAll('rect.legend')
      .data(colorList)
    .enter().append('rect')
      .attr('class', 'legend')
      .attr('x', function(d,i){return xFrom + legendW*i})
      .attr('y', height+ rowHeight*2 + 38)
      .attr('width', legendW)
      .attr('height', legendW*.25)
      .style('fill', function(d){return d});

    legendW = legendW * 1.5;
    xFrom =  (width - colorList.length*legendW )/2
    svg.selectAll('text.legend')
      .data(['열심히', '', '빡세게'])
    .enter().append('text')
      .attr('class', 'legend')
      .attr('x', function(d,i){
        return xFrom + legendW*i + legendW*.5;
      })
      .attr('y', height+ rowHeight*2 + 38 + legendW*.5)
      .attr('text-anchor', 'middle')
